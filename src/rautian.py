...
def calc_rautian(nu, nu0, doppler_width, lorentz_width, narrowing, shift):
    ...
    if b:
        shape = (np.sqrt(np.log(2) / np.pi) / doppler_width) * (w / (1 - (np.sqrt(np.pi) * b * w))).real
        return shape
    return ((np.sqrt(np.log(2) / np.pi) / doppler_width) * w.real )
